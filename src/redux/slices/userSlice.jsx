...
export const loginUser = createAsyncThunk('login/loginUser', async (userData) => {
  try {
    const response = await axios.post('https://react-rails-api-he81.onrender.com/api/users/login', userData);
...
export const registerUser = createAsyncThunk('register/registerUser', async (userData, { rejectWithValue }) => {
  try {
    const response = await axios.post('https://react-rails-api-he81.onrender.com/api/users', userData);
    localStorage.setItem('user', JSON.stringify(response.data.data));
